...
data_path = Path("C:/research/TJM/TransformerJM/datasets/nonlin/r_data_nlmv1.csv")
...
model = Transformer(d_long=1, d_base=2, d_model=32, nhead=4,
...
for epoch in range(n_epoch):
    ...
    for batch in range(0, len(train_id), batch_size):
        ...
        batch_long, batch_base, batch_mask, batch_e, batch_t, obs_time = get_tensors(batch_data.copy(),long=["Y"],base=["X1","X2"])
    ...
    with torch.no_grad():  # Disable gradient calculation
        for batch in range(0, len(test_id), batch_size):
            ...
            batch_long, batch_base, batch_mask, batch_e, batch_t, obs_time = get_tensors(
                batch_data.copy(), long=["Y"], base=["X1","X2"])
            batch_long_inp = batch_long[:, :-1, :]
            batch_long_out = batch_long[:, 1:, :]
            batch_base = batch_base[:, :-1, :]
            batch_mask_inp = get_mask(batch_mask[:, :-1])
...
tmp_long, tmp_base, tmp_mask, e_tmp, t_tmp, obs_time_1 = get_tensors(tmp_data.copy(),long=["Y"],base=["X1","X2"])
train_long, train_base, train_mask, e_train, t_train, obs_time = get_tensors(train_data.copy(),long=["Y"],base=["X1","X2"])
...
output_dir = "C:/research/TJM/TransformerJM/datasets/prednl/"  # Replace with your desired output directory
...
for dataset_file in os.listdir("C:/research/TJM/TransformerJM/datasets/nonlin"):  # Replace with your dataset directory
    if dataset_file.endswith(".csv"):
        dataset_path = os.path.join("C:/research/TJM/TransformerJM/datasets/nonlin", dataset_file)
        ...
        for LT in landmark_times:
            ...
            tmp_long, tmp_base, tmp_mask, e_tmp, t_tmp, obs_time_1 = get_tensors(
                tmp_data.copy(), long=["Y"], base=["X1", "X2"]
            )
